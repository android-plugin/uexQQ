...
public class EUExQQ extends EUExBase {
	...
	public static String mAppid = "100840828";
	...
	private class BaseUiListener implements IUiListener {
		...
		public void onError(UiError e) {
			loginCallBack(EUExCallback.F_C_FAILED, "errorCode:" + e.errorCode
					+ "," + e.errorDetail);
		}
	...
	public boolean isQQInstalled(String[] params) {
		...
		jsCallback(CB_INSTALLIED, 0, EUExCallback.F_C_TEXT, isInstalled ? 0 : 1);
		return isInstalled;
	}
	...
	public void loginCallBack(int ret, Object data) {
		...
		jsCallbackAsyn(CB_LOGIN, 0, EUExCallback.F_C_JSON, json.toString());
	}
	...
	private void parseWebImgData(String jsonData) {
		try {
			...
			if (json.has(QQShare.SHARE_TO_QQ_EXT_INT)) {
				params.putInt(QQShare.SHARE_TO_QQ_EXT_INT, Integer.valueOf(json
						.get(QQShare.SHARE_TO_QQ_EXT_INT).toString()));
			}
	...
	private void parseLocalImgData(String jsonData) {
		try {
			...
			JSONObject json = new JSONObject(jsonData);
			String imageLocalUrl = json
					.get(QQShare.SHARE_TO_QQ_IMAGE_LOCAL_URL).toString();
			// 需要分享的本地图片路径。
			params.putString(QQShare.SHARE_TO_QQ_IMAGE_LOCAL_URL,
					getImageUrl(imageLocalUrl));
	...
	private void parseAudioData(String jsonData) {
		try {
			...
			String targetUrl = json.get(QQShare.SHARE_TO_QQ_TARGET_URL)
					.toString();
			params.putString(QQShare.SHARE_TO_QQ_TARGET_URL, targetUrl);// 这条分享消息被好友点击后的跳转URL。
			String audio_url = json.get(QQShare.SHARE_TO_QQ_AUDIO_URL)
					.toString();
	...
	private void parseImgTextToQZoneData(String jsonData) {
		try {
			...
			params.putString(QzoneShare.SHARE_TO_QQ_TITLE, title);// 分享的标题,
																	// 最长30个字符。
			String targetUrl = json.get(QzoneShare.SHARE_TO_QQ_TARGET_URL)
					.toString();
			...
			if (json.has(QzoneShare.SHARE_TO_QQ_SUMMARY)) {
				String summary = json.get(QzoneShare.SHARE_TO_QQ_SUMMARY)
						.toString();
			...
			if (json.has(QzoneShare.SHARE_TO_QQ_IMAGE_URL)) {
				...
				JSONArray jsonArray = json
						.getJSONArray(QzoneShare.SHARE_TO_QQ_IMAGE_URL);
